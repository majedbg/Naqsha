...
| 5-rev | ⚠️ **DECISION 5 REVERSED BY MAJED, 2026-07-29 — the hard tier takes `max(R_root, R_tight)`.** Decision 5 restated host/boundary containment against the tight disc alone; measurement showed that direction is backwards. Both bounds are **independently sound containment certificates** — art inside the root disc *or* inside the tight disc each guarantee art inside the container — so the less restrictive of the two is safe to take, and is never worse than either. Root wins at an undisplaced centre (`H` vs `0.829·H` for `slice100`); tight can win when jitter displaces the glyph away from its offset direction. **The neighbour term is unchanged** — it keeps the tight disc, and the whole 4× recovery lives there. Supersedes decisions 5, 5b, 5c and 5d; §7z 5e-obs still stands. |
| 5-rev-built | **BUILT.** The tight arm's hard tier now takes `Math.max` of the two bounds on both channels — `boundaryCap = max(capOf(parts.boundary), capOf(boundaryLimit(…)))` and `hostCap = max(margin·max(0, hostRadius − d), capOf(hostLimit(…)))` — with the root-side expressions COPIED from the legacy arm rather than shared with it (ADR-0005 is byte-identity, not algebra). 5c's rejection rule restates as "neither bound permits a positive `R`", which is a single guard because the two conditions coincide (`d ≥ H` ⇔ `hostLimit` returns `-1`). Measured after: `slice100` in a cell at an undisplaced centre is back to `1.000×` *bit-identically*, 0 of 62 glyphs shrink, and a jittered `slice100` leaning back across its cell measures `6.14 → 14.81` with the TIGHT bound winning the max. ⚠️ **What survives 5-rev is ART inside the container, not RESERVE inside the container** — when the root bound wins, the committed tight disc legitimately pokes outside the cell, because it bulges past the ink. Any containment assertion must be made of the flattened art. ⚠️ **DATA FINDING, pre-existing:** taking the root bound makes containment rest on `viewRadius ≥ max\|p − root\|`, which `glyphFootprint.test.js` never recomputes (it recomputes only `footprintCenter`/`footprintRadius`). **Six of the 62 understate their own reach** at `flattenPathD` tol 0.05 — worst `slice85`, 5.3e-4 relative — so their ink crosses the container wall by well under a thousandth. This is not caused by 5-rev: the root arm has always reserved `packedRadius` and drawn out to `reach × packedRadius`. Pinned as "the tight arm never puts ink further out than the root law already does, and the absolute slack stays below `H × 1e-3`". Worth its own ticket if the numbers are ever meant to be exact. |
| 7g | **The reserve must apply `root.angle`, and the composition is DERIVED, not guessed.** `placementMatrix` composes `M = T(P)·R(θ)·S(sx,sy)·R(−φ)·T(−root)` with `φ = root.angle` (`instancing.js:63`), so a root-relative art point `q` lands at `P + R(θ)·diag(σ,1)·s·R(−φ)·q`, `σ = −1` when flipped. `R(θ)·R(−φ) = R(θ−φ)`; and `diag(−1,1)·R(−φ) = R(φ)·diag(−1,1)`, so the flipped case is `R(θ+φ)·(−f̂c.x, f̂c.y)`. The shipped `u = Rot(θ)·f̂c` is the `φ = 0` case only. Measured before the fix: `slice100` given a 90° growth turn put ink **74.9%** of the reserve radius outside its committed disc, and a synthetic hook glyph swept over φ ∈ {0,30,90,−45,137,180} × θ ∈ {0,37,90,180,270} × flip reached **175.2%**; after, both are 0 (worst 8.2e-13 %). `φ === 0` short-circuits to the bare `rotation`, mirroring `placementMatrix`'s own default-root short-circuit, so all 62 built-ins stay bit-identical. Regression-tested with SYNTHETIC glyphs — shipped data is all `angle: 0` and cannot exercise it. |
| 7e-built | **BUILT and MEASURED (#207).** The default flip lives in the layer CONSTRUCTORS — `starterChips.js`, `defaultBinding.js` and `motifLayer.js`'s `normalizeBinding`, which stamps `'tight'` only when absent — while `PLACEMENT_DEFAULTS.sizing.footprint` stays `'root'` and is now EXPORTED so `footprintScope.isTightFootprint` reads the engine's own default instead of a second copy of it. Absent has to keep meaning root: `pinFootprint` deliberately leaves a pre-v2 layer that never carried a `sizing` object alone, and that is the one hole the pin cannot cover. Population measured over the 62 built-ins × 4 scenarios (free-packed and `hostRadius` cells, each with and without jitter), 420×340, at the new-layer tail `size 18 / min 3 / margin 0.85`: **29388 → 29613 placements**, **total drawn area +42.4%**, **`below-floor` + `no-fit` 372 → 147**. 21927 grew, 2168 shrank, 5280 unchanged; **0 unexplained shrinks**, and 0 cell-hosted glyphs below their root-law size (5-rev holds). ⚠️ **5e-obs UNDERSTATES the shrink mechanism, and its supporting claim is false.** "Per obstacle the tight limit is never worse than the root limit" does not hold, and greedy redistribution accounts for only **1917 of the 2168** shrinks. The remaining 251 split three ways, each bounded and each sound: **232** where the captor's `drawnRadius` is UNCHANGED but its reserve now reaches further toward the victim — the MEC bulges past the ink by exactly `\|f̂c\| + f̂r ≥ 1`, the very inequality 5-rev proves, so the loss is bounded by `R·(\|f̂c\| + f̂r − 1)` and was measured at 3.655 against a bound of 3.704 for `slice100` at `R = 18`; **14** where the VICTIM's own reserve leans at its captor, floored at `R_root / (\|f̂c\| + f̂r)`; and **5** where the captor is a glyph the ROOT law rejected outright and the tight law placed. All three are the same root cause — the minimal enclosing circle is a looser envelope than the root disc in the leaning direction and a far tighter one everywhere else — which is why the population gains 42% while individuals redistribute. |
