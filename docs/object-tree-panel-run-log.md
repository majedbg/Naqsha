...
### Known seams / un-delivered spec behaviors (for the user to decide)
1. **§3.2 responsive dice-hide unthreaded.** Implemented as a `compact` prop (default false), unit-tested, but nothing passes it — so the dice does not actually hide in the live 200–240px band. Fix options: install a Tailwind container-query plugin and use `@container` (spec's preferred approach), lift panel width to Studio, or leave it. Low risk (40px band).
2. **§4 RowMenu `anchorNearBottom` (flip-up) unwired.** The flip-up *capability* exists + is unit-tested in RowMenu (WI-4), but `LayerTree.jsx` renders `<RowMenu>` without computing `anchorNearBottom`, so the menu always opens downward. **Deliberately not wired with a naive heuristic:** the object-tree panel is full shell height and the layer count is capped at 6, so rows sit near the panel's TOP — an index-based `anchorNearBottom` would cause wrong-direction upward flips in the common case (a regression), and the real clip case (long, bottom-scrolled list) doesn't arise under the 6-layer cap. A correct fix would measure `getBoundingClientRect` vs the scroll container's bottom on open (not meaningfully testable in jsdom). Recommend leaving unwired unless the layer cap grows or the panel becomes scrollable.

### Verification scope (honesty note)
All verification was **`npm test` (Vitest/jsdom) + `npm run build` + ESLint** — green throughout. **No live browser pass** was run. The features carrying the most jsdom-vs-reality risk were therefore not exercised in a real browser: drag-resize feel + the divider hover/drag accent (WI-3), RowMenu real-layout positioning/clipping (WI-4/5, see seam #2), rename input focus + select-all (WI-5), and the responsive collapse (seam #1). The runbook makes the dev-server pass optional; offered to the user if they want it.
