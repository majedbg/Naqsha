...
function ReadyItem({ status, label, detail }) {
  const color =
    status === 'ok' ? 'text-tone-ok'
    : status === 'warn' ? 'text-tone-mild'
    : 'text-ink-soft';
    ...
    <div className="space-y-5">
              ...
              <button
                ...
                className={`text-left p-2 rounded-md border transition-colors ${
                  active
                    ...
                    : disabled
                      ? 'border-paper-warm bg-paper-warm text-ink-soft cursor-not-allowed'
                      : 'border-paper-warm bg-paper-warm text-ink hover:border-ink-soft'
                }`}
                title={f.hint}
              >
                ...
                <div className="text-[10px] text-ink-soft mt-0.5 leading-snug">{f.hint}</div>
              </button>
            );
          })}
        </div>
      ...
      <section className="space-y-2">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="text-[10px] text-ink-soft hover:text-ink-soft transition-colors"
        ...
        <div className="flex items-center gap-1.5">
          <input
            ...
            className="flex-1 bg-paper-warm text-ink text-[11px] font-mono px-2 py-1.5 rounded border border-hairline outline-none focus:border-violet"
          />
          <span className="text-[10px] text-ink-soft">.svg</span>
        </div>
      ...
      <section className="space-y-2">
        <button
          ...
          className="w-full py-2.5 text-sm font-medium rounded bg-accent text-ink hover:bg-saffron-hover transition-colors"
        >
        ...
        <div className="flex items-center justify-between gap-2">
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            />
            <span className="text-[11px] text-ink-soft">Include hidden layers</span>
          </label>
      ...
      <section className="space-y-2">
        ...
        <ul className="space-y-1 p-2.5 rounded-md bg-paper border border-paper-warm">
          {readyItems.map((r, i) => (
        ...
        {outputMode === 'laser' && hasUntaggedLayers && onTabChange && (
          <button
            ...
            className="text-[11px] text-tone-mild hover:text-tone-mild transition-colors"
          >
      ...
      <section className="space-y-2 border-t border-hairline pt-4">
        ...
        {user ? (
          <div className="space-y-2">
            <button
              ...
              className="w-full py-2 text-[12px] font-medium rounded border border-violet/40 text-accent hover:bg-accent/10 transition-colors"
            >
              Save to Cloud
            </button>
            <button
              onClick={onOpenCloudDesigns}
              className="w-full py-1.5 text-[11px] text-ink-soft hover:text-saffron transition-colors"
            >
              My Cloud Designs
            </button>
          </div>
        ) : (
          <div className="rounded border border-hairline bg-paper-warm p-3 text-center">
        ...
        {limits.localStorage && (
          <button
            ...
            className="w-full py-1.5 text-[11px] text-ink-soft hover:text-ink transition-colors"
          >
            Save Local Backup
          </button>
        )}
