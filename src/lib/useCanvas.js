...
export default function useCanvas(
  ...
  const renderAll = useCallback(() => {
    ...
    const hostGeometry = collectMotifHostGeometry(layers, (host) => {
      ...
      // the visible flow lines / contours (the D1 fail-unsafe ~24px drift, #103).
      // Inject the SAME resolved modulation so the probe's geometry matches paint,
      // symmetric with the paint pass:
      //   • grid warped → draws bezierVertex (capturePolylines does NOT record) →
      //     empty hostPaths → nothing placed, the intended single-axis-vine deferral.
      //   • flowfield / topographic warped → emit plain ctx.vertex → captured on the
      //     warped flow lines / contours → anchors track the painted geometry.
